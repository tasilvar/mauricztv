...
namespace bpmj\wpidea\sales\product\vo;

use Exception;

class Product_Sales_Status
{
    ...
    public function __construct(string $status)
    {
        if (!in_array($status, self::VALID_STATUS, true)) {
            throw new Exception('Invalid product sales status provided!');
        }

        $this->status = $status;
    }

    public function equals(Product_Sales_Status $other_status): bool
    {
