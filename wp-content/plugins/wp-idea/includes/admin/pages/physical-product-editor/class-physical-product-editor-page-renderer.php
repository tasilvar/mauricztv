...
class Physical_Product_Editor_Page_Renderer extends Abstract_Page_Renderer
{
    public const PHYSICAL_PRODUCT_ID_QUERY_ARG_NAME = 'edit_physical_product_id';
    ...
    public function get_rendered_page(): string
    {
        ...
        return $this->view_provider->get_admin(Admin_View_Names::PRODUCTS_EDITOR, [
            'page_title' => $this->translator->translate('physical_product_editor.page_title'),
            'sections' => $this->get_sections(),
            'get_tab_ajax_url' => $this->prepare_get_tab_ajax_url(),
            'preview_text' => $this->translator->translate('physical_product_editor.preview_button'),
    ...
    private function get_sections(): array
    {
        $sections = [
            [
                'id' => 'general',
                'title' => $this->translator->translate('physical_product_editor.sections.general')
            ],
            [
                'id' => 'link_generator',
                'title' => $this->translator->translate('physical_product_editor.sections.link_generator')
            ],
        ];
        ...
        if ($this->settings->get(Settings_Const::INVOICES_ENABLED)) {
            $sections[] = [
                'id' => 'invoices',
                'title' => $this->translator->translate('physical_product_editor.sections.invoices')
            ];
        }
        ...
        if ($this->is_mailer_integration_enabled()) {
            $sections[] = [
                'id' => 'mailings',
                'title' => $this->translator->translate('physical_product_editor.sections.mailings')
            ];
        }
        ...
        if ($this->settings->get(Settings_Const::SELL_DISCOUNTS_ENABLED)) {
            $sections[] = [
                'id' => 'discount_code',
                'title' => $this->translator->translate('physical_product_editor.sections.discount_code')
            ];
        }
    ...
    {
        $id = $this->current_request->get_query_arg(self::PHYSICAL_PRODUCT_ID_QUERY_ARG_NAME);
    ...
    private function prepare_get_tab_ajax_url(): string
    {
        return $this->url_generator->generate(Admin_Settings_Fields_Ajax_Controller::class, 'get_tab_content', [
            ...
            self::PHYSICAL_PRODUCT_ID_QUERY_ARG_NAME => $this->get_product_id()
        ]);
    }
