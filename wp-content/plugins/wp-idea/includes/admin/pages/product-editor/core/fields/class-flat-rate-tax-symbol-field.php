...
class Flat_Rate_Tax_Symbol_Field extends Abstract_Setting_Field
{
                ...
                <div class="single-input-wrapper ">
                    <?php
                     echo'<select name="'.$this->get_name().'" id="'.$this->get_name().$price_id.'" data-initial-value="none" class="single-field wpi-select-field">
                           '.$this->get_options_string($variable_price['flat_rate_tax_symbol'] ?? '', (int)$price_id).'
                         </select>';
                    ?>
                </div>
            </div>
    ...
    private function get_options_string(?string $flat_rate_tax_symbol, ?int $id = null): string
    {
        $html = '';
        foreach ($this->options as $value => $label) {
            $selected = '';
            if ((string)$value === $flat_rate_tax_symbol) {
