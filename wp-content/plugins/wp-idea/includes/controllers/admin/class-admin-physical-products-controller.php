...
class Admin_Physical_Products_Controller extends Ajax_Controller
{
    ...
    public function get_popup_create_physical_products_action(Current_Request $current_request): string
    {
        return $this->success(
            [
                'content' => $this->admin_view('/popup/create-product-popup-content', [
                    'title' => $this->translator->translate('physical_products.actions.create_product'),
                    'form_id' => 'physical_products_popup_editor',
                    'create_product_url' => $this->get_create_physical_products_url(),
    ...
    public function delete_action(Current_Request $current_request): string
    {
        ...
        if ($this->bundles_app_service->product_belongs_to_any_bundle($product_id)) {
            return $this->fail($this->translator->translate('physical_products.actions.delete.info'));
        }

        $this->physical_products_app_service->delete_product($product_id);

        return $this->success(
            [
                'message' => $this->translator->translate('physical_products.actions.delete.success')
            ]
        );
    }
        ...
        return $this->success(
            [
                'content' => $this->admin_view('/services/list/purchase-links-popup-content', [
                    'title' => $this->translator->translate('physical_products.popup.purchase_links.title'),
    ...
    public function disable_sales_action(Current_Request $current_request): string
    {
        ...
        $this->physical_products_app_service->toggle_sales(new Product_ID($id));
    ...
    private function get_create_physical_products_url(): string
    {
        return $this->url_generator->generate(Admin_Creator_Controller::class, 'create_physical_product', [
