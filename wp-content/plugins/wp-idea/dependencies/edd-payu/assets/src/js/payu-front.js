+ function ( $ ) {
	...
	function init() {
		...
		$( document ).on( 'click', '.edd-payu-submit-button:not(#edd-payu-button-pay-for-all-standard)', function ( e ) {
			...
			var $button = $( this );
			if ( 'submit_payu_opening_popup' === $button.attr( 'type' ) ) {
				// PayU popup should open in a moment - we can switch type back to 'submit'
			...
			$.post( bpmj_eddpayu.ajax_url, post_params, function ( data ) {
				if ( 'success' === $.trim( data ) ) {
					...
					var interval = setInterval( function () {
						...
						if ( window.PayU && window.PayU.Merchant && window.PayU.Merchant.$button.get( 0 ) === button ) {
							clearInterval( interval );
							// Change the type for a moment to prevent firing EDD default action for submit buttons
							$button.attr( 'type', 'submit_payu_opening_popup' );
							$button.click();
						}
					}, 50 );
				} else {
