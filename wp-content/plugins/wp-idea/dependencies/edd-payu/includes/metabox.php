		...
		if ( edd_download_is_recurring( $download_id ) ) {
            ...
            <p>
                ...
                <label>
	                ...
	                <?php _e( 'Check this box to enable recurring payments for this product', BPMJ_EDDPAYU_DOMAIN ); ?>
                </label>
            </p>
			<?php
		}
...
if ( ! function_exists( 'edd_meta_box_recurring_payments_interval' ) ) {
	function edd_meta_box_recurring_payments_interval( $download_id = 0 ) {
		if ( edd_download_is_recurring( $download_id ) ) {
            ...
            <p>
                <label>
                    <select name="_edd_recurring_payments_interval_number" <?php disabled( ! $recurring_payments_possible ); ?>>
                ...
                <label>
                    <select name="_edd_recurring_payments_interval_unit" <?php disabled( ! $recurring_payments_possible ); ?>>
	            ...
	            <?php _e( 'Set the interval between recurring payments for this item', BPMJ_EDDPAYU_DOMAIN ); ?>
            </p>
			<?php
		}
	}
...
if ( ! function_exists( 'edd_meta_box_recurring_payments_switch_variable_head' ) ) {
	function edd_meta_box_recurring_payments_switch_variable_head( $download_id = 0 ) {
		if ( edd_download_is_recurring( $download_id ) ) {
			?>
            <th><?php _e( 'Recurring payments', BPMJ_EDDPAYU_DOMAIN ); ?></th>
			<?php
		}
	}
			...
			<td>
				...
				<label>
					<input type="checkbox"
					       ...
					       value="1"
						<?php disabled( ! $recurring_payments_possible ); ?>/>
					<?php _e( 'Enable', BPMJ_EDDPAYU_DOMAIN ); ?>
				</label>
			</td>
			<?php
		}
...
if ( ! function_exists( 'edd_meta_box_recurring_payments_interval_variable_head' ) ) {
	function edd_meta_box_recurring_payments_interval_variable_head( $download_id = 0 ) {
		if ( edd_download_is_recurring( $download_id ) ) {
			?>
            <th><?php _e( 'Interval', BPMJ_EDDPAYU_DOMAIN ); ?></th>
			<?php
		}
	}
