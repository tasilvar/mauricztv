	...
	try {
		...
		if ( has_filter( 'edds_create_charge_args' ) ) {
			...
			foreach ( $compatible_keys as $compatible_key ) {
				$value = ! empty( $old_charge_args[ $compatible_key ] )
					? $old_charge_args[ $compatible_key ]
					: $intent_args[ $compatible_key ];
				...
				switch ( $compatible_key ) {
					...
					default:
						$intent_args[ $compatible_key ] = $value;
				}
...
function edds_create_payment() {
    ...
    add_filter( 'edd_auto_register_login_user', fn() => true);
