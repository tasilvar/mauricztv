...
class EDDSP_Sale_Price {
	...
	public function maybe_display_sale_price( $price, $download_id ) {
        ...
        $sale_price = apply_filters('get_sale_price', get_post_meta( $download_id, 'edd_sale_price', true ), $price, $download_id);
	 ...
	 * @param	int		$download_id	ID of the download we're changing the price for.
	 * @return	array					Array of new prices, if the variant is in sale this will be the sale price.
	 */
	public function maybe_display_variable_sale_prices( $prices, $download_id ) {
		...
		return apply_filters('get_sale_price_for_variant', $prices, $download_id);
	...
	public function edd_price_maybe_display_sale_price( $formatted_price, $download_id, $price, $price_id ) {
		...
		if ( edd_has_variable_prices( $download_id ) ) :
			...
			if ( false !== $price_id && isset( $prices[ $price_id ] ) ) {
				$regular_price 	= isset( $prices[ $price_id ]['regular_amount'] ) ? (float) $prices[ $price_id ]['regular_amount'] : (float) $prices[ $price_id ]['amount'];
            ...
            $sale_price = apply_filters('get_sale_price', get_post_meta( $download_id, 'edd_sale_price', true ), $regular_price, $download_id);
	...
	public function maybe_display_sale_price_text( $args ) {
		...
		if ( $args['price'] && $args['price'] !== 'no' ) {
            ...
            $sale_price = apply_filters('get_sale_price', get_post_meta( $args['download_id'], 'edd_sale_price', true ), $regular_price, $args['download_id']);
		}
	...
	public function checkout_maybe_display_sale_price( $label, $item_id, $options ) {
		...
		if ( $download->has_variable_prices() ) :
            ...
            $sale_price = apply_filters('get_sale_price', get_post_meta( $item_id, 'edd_sale_price', true ), $regular_price, $item_id);
		endif;
		...
		$price 			= '<ins>' . edd_currency_filter( edd_format_amount( $price ) ) . '</ins>';

		return $price . ' ' . $regular_price . $label;
			...
			<ul>
				...
				if ( $prices ) :
					...
					foreach ( $prices as $key => $price ) :
						...
						?><li id="edd_price_option_<?php echo $download_id . '_' . sanitize_key( $price['name'] ); ?>">
							<label for="<?php echo esc_attr( 'edd_price_option_' . $download_id . '_' . $key ); ?>">
								<input type="<?php echo $type; ?>" <?php checked( apply_filters( 'edd_price_option_checked', $checked_key, $download_id, $key ), $key ); ?>
                                    ...
                                    <?php View_Hooks::run(View_Hooks::RENDER_INLINE_ELEMENTS_IN_ADD_TO_CART_VARIANT_OPTIONS, ['product_id' => $download_id, 'variant' => esc_attr( $key )]) ?>
									...
									<span class='edd_price_option_wrap'>
										...
										if ( isset( $price['sale_price'] ) && ! empty( $price['sale_price'] ) && isset( $price['regular_amount'] ) ) :
											?><span class="edd_price_option_price regular_price"><del><?php
