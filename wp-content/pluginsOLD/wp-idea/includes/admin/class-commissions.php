...
/**
 *
 * The class responsible for integration with EDD Commissions plugin
 *
 */
...
class Commissions
{
    public function __construct()
    {
        ...
        add_action('save_post', array($this, 'bpmj_wpidea_download_meta_box_save'), 10, 1);
        add_action('add_meta_boxes', array($this, 'bpmj_wpidea_add_commission_meta_box'), 100);

        add_filter('eddc-admin-script-screens', array($this, 'bpmj_wpidea_filter_allowed_edd_commissions_screens'), 100);
    }

    // Add Courses to screens that EDD Commissions scripts are allowed on
    function bpmj_wpidea_filter_allowed_edd_commissions_screens($allowed_screens)
    {
        $allowed_screens[] = 'courses';
        return $allowed_screens;
    }

    // add meta box
    function bpmj_wpidea_add_commission_meta_box()
    {
        if (current_user_can('manage_shop_settings')) {
            add_meta_box('edd_download_commissions', __('Commission', 'edd'), array($this, 'bpmj_wpidea_render_commissions_meta_box'), 'courses', 'normal', 'low'); //#WPIDEA-MOD
        }
    }

    // find download id coresponging to current course
    function get_associated_download_id($post)
    {
        if ($post->post_type == 'courses') {
            $associatedDownload = get_posts(array('post_name' => $post->post_name, 'post_type' => 'download'));
            if ($associatedDownload) return $associatedDownload[0]->ID;
            return $post->ID;
        }
        return $post->ID;
    }
     ...
     *
     * Function comes from EDD Commissions plugin
     */
    function bpmj_wpidea_render_commissions_meta_box()
    {
        global $post;
        ...
        // #WPIDEA-MOD
        if ($post->post_type == 'courses') {
            $post = get_post($this->get_associated_download_id($post));
        }
        ...
        ?>
        <input type="hidden" name="edd_download_commission_meta_box_nonce"
        ...
        <table class="form-table">
            ...
            <tr id="eddc_commission_enable_wrapper">
                <td class="edd_field_type_text" colspan="2">
                    ...
                    <input type="checkbox" name="edd_commisions_enabled" id="edd_commisions_enabled"
                           value="1" <?php checked(true, $enabled, true); ?>/>&nbsp;
            ...
            <tr <?php echo $display; ?> class="eddc_toggled_row" id="eddc_commission_type_wrapper">
                <td class="edd_field_type_select">
                    ...
                    <span alt="f223" class="edd-help-tip dashicons dashicons-editor-help"
                          title="<strong><?php _e('Type', 'eddc'); ?></strong>: <?php _e('With commissions enabled, you will need to specify who to assign commissions to. Commissions can be given based on a percentage of the purchase cost, or at a flat rate.', 'eddc'); ?>"></span><br/>
                    <p><?php
                        ...
                        foreach ($commission_types as $commission_type => $commission_pretty_string) {
                            ...
                            <span id="eddc_type_<?php echo $commission_type; ?>_wrapper"><input
                                        id="eddc_type_<?php echo $commission_type; ?>" type="radio"
                                        name="edd_commission_settings[type]"
                                        value="<?php echo $commission_type; ?>" <?php checked($type, $commission_type, true); ?>/> <?php echo $commission_pretty_string;
                                ?></span><br/><?php
                        }
                        ?>
                    </p>
            ...
            <table class="widefat edd_repeatable_table" width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <th class="eddc-commission-rate-rate">
                        <?php _e('Rate', 'eddc'); ?>
                        <span alt="f223" class="edd-help-tip dashicons dashicons-editor-help"
                              title="<strong> <?php _e('Rate', 'eddc'); ?></strong>:&nbsp;
                ...
                <?php if (!empty($rates)) : ?>
                    <?php foreach ($rates as $key => $value) : ?>
                        <tr class="edd_repeatable_upload_wrapper edd_repeatable_row"
                            ...
                            <td>
                                <input type="text" name="edd_commission_settings[rates][<?php echo $key; ?>][amount]"
                                       id="edd_commission_amount_<?php echo $key; ?>"
                                       value="<?php echo $value['amount']; ?>"
                                       placeholder="<?php _e('Rate for this user', 'eddc'); ?>"/>
                            </td>
                            <td>
                                <a href="#" class="edd_commissions_remove_repeatable"><span
                                            class="dashicons dashicons-dismiss"></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
                    <tr class="edd_repeatable_upload_wrapper edd_repeatable_row" data-key="1">
                        ...
                        <td>
                            <input type="text" name="edd_commission_settings[rates][1][amount]"
                                   id="edd_commission_amount_1"
                                   placeholder=" <?php _e('Rate for this user', 'eddc'); ?>"/>
                        </td>
                        <td>
                            <a href="#" class="edd_commissions_remove_repeatable" data-type="commission"><span
                ...
                <tr>
                    <td class="submit" colspan="4" style="float: none; clear:both; background: #fff;">
                        <a class="button-secondary edd_commission_rates_add_repeatable"
                           style="margin: 6px 0 10px;"><?php _e('Add New Commission Rate', 'eddc'); ?></a>
                    </td>
                </tr>
                </tbody>
            </table>
     ...
     * @return int
     */
    public function bpmj_wpidea_download_meta_box_save($post_id)
    {
        global $post;
        ...
        // #WPIDEA-MOD
        if ($post->post_type == 'courses') {
            $post_id = $this->get_associated_download_id($post);
        }
        // verify nonce
        ...
        if (isset($_POST['post_type']) && 'download' != $_POST['post_type'] && 'courses' != $_POST['post_type']) {
            return $post_id;
        }
