...
if ( $id !== 'none' ) {
	...
	echo '<script> window.location.replace("' . admin_url( 'admin.php?page=wp-idea-settings' ) . '"); </script>';
}
?>
...
<form id="bpmj-eddpc-renewal-form" action="" method="post">
            ...
            <tr>
                ...
                <td>
                    ...
                    <label><input type="checkbox" name="charge-mode[]" value="manual"
					              <?php if ( in_array( 'manual', $charge_modes ) ): ?>checked="checked"<?php endif; ?>/> <?php _e( 'manual payments', 'edd-paid-content' ); ?>
                    </label>
					...
					<?php else: ?>
                        ...
                        <li><code>{direct_payment_link}</code>
                            - <?php printf( __( 'Direct payment link'
							                    . ' - in the case of automatic payments, the following text is substituted instead of the link: %s', 'edd-paid-content' ),
    ...
    <p class="submit">
        ...
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=' . \bpmj\wpidea\admin\menu\Admin_Menu_Item_Slug::SETTINGS . '&autofocus=messages' ) ); ?>"
