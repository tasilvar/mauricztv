...
class Variable_Prices_Field extends Abstract_Setting_Field
{
    ...
    private function field_variable_pricing_table(int $post_id, array $variable_prices): string
    {
        ...
        <div style="width:100%;">
            <table class="table_variable_prices">
                <thead>
                <tr>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.id') ?></th>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.name') ?></th>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.price') ?></th>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.availability') ?></th>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.access_on') ?></th>
                    <th><?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.head.recurring_payments') ?></th>
                </tr>
                </thead>
        ...
        <div style="width:100%; clear:both;">
            <br />
            <button type="button" data-action="edit-variable-prices" data-post-id="<?php echo $post_id; ?>" class="wpi-button wpi-button--clean configuration-button" style="float:right">
                <?= Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.edit_variants') ?>
            </button>
        </div>
        <?php
    ...
    protected function get_variable_prices_html(array $variable_prices): string
    {
        ...
        foreach ( $variable_prices as $price_id => $variable_price ) {
            ...
            $access_time = $variable_price[self::ACCESS_TIME]
                ? $variable_price[self::ACCESS_TIME] .' '. Translator_Static_Helper::translate('course_editor.sections.general.access_time_unit.option.'.$variable_price[self::ACCESS_TIME_UNIT]) : '-';

            $recurring_payments_enabled = $variable_price[self::RECURRING_PAYMENTS_ENABLED]
                ? Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.body.'.self::RECURRING_PAYMENTS_ENABLED.'.yes')
                : Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.body.'.self::RECURRING_PAYMENTS_ENABLED.'.no');
        ...
        if (!$html) {
            $html = '<tr><td colspan="6" style="text-align:center;">'. Translator_Static_Helper::translate('course_editor.sections.general.variable_prices.table.body.set_price_variants') .'</td></tr>';
        }
