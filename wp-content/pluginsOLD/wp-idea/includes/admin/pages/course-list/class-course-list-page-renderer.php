...
class Course_List_Page_Renderer extends Abstract_Page_Renderer
{

    private Interface_View_Provider $view_provider;
    private Interface_Translator $translator;
    private Dynamic_Tables_Module $dynamic_tables_module;
    private Course_List_Table_Config_Provider $config_provider;
    ...
    public function __construct(
        ...
        Course_List_Table_Config_Provider $config_provider,
    ...
    {
        return $this->view_provider->get_admin(Admin_View_Names::COURSES_LIST_PAGE, [
            'page_title' => $this->translator->translate('course_list.page_title'),
            'table' => $this->prepare_table(),
            'no_access_to_cloning' => $this->maybe_get_no_access_to_cloning_message()
        ]);
    }
    ...
    private function maybe_get_no_access_to_cloning_message(): ?string
    {
        if($this->packages_api->has_access_to_feature(Packages::FEAT_COURSE_CLONING)) {
            return null;
        }

        return $this->packages_api->render_no_access_to_feature_info(
            Packages::FEAT_COURSE_CLONING,
            $this->translator->translate('services.actions.duplicate.not_available_in_your_package')
        );
    }
}
