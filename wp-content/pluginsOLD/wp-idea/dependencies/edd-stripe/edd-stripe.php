...
Version: 2.7.3.1 (based on 2.7.3)
...
class EDD_Stripe implements Interface_External_Service_Integration {
use Trait_External_Service_Integration;

    const SERVICE_NAME = 'Stripe';
	...
	private function setup_constants() {
		...
		define( 'EDD_STRIPE_VERSION', '2.7.3.1' );
	...
	private function includes() {
		if ( ! class_exists( 'Stripe\Stripe' ) ) {
			require_once EDDS_PLUGIN_DIR . '/vendor/stripe/stripe-php/init.php';
		}
    ...
    public function check_connection(): bool
    {
        ...
        $key = edd_get_option( 'live_secret_key', '' );
        if(empty($key)){
            $key = edd_get_option( 'test_secret_key', '' );
        }

        $client = new Http_Client();
        $response = $client->create_request()
            ->set_url(self::API_URL.'/charges')
        ...
        if(isset($body->object) && $body->object == 'list'){
            return true;
        }
