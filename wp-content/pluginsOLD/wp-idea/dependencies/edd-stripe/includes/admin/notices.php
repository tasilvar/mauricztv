...
function edds_admin_notices_register() {
	...
	if ( ! $registry ) {
		return new WP_Error( 'edds-invalid-registry', esc_html__( 'Unable to locate registry', 'edd-stripe' ) );
	}
			...
			array(
				'message'     => '<p>' . wp_kses(
					sprintf(
						...
						__( 'The Stripe extension for Easy Digital Downloads supports Stripe Connect for easier setup and improved security. %1$sClick here%2$s to learn more about connecting your Stripe account.', 'edds' ),
						'<a href="' . esc_url( admin_url( 'edit.php?post_type=download&page=edd-settings&tab=gateways&section=edd-stripe' ) ) . '">',
						'</a>'
					),
					array(
						'a' => array(
			...
			array(
				'message'     => '<p>' . wp_kses(
					sprintf(
						...
						__( '%1$sCredit card payments with Stripe are currently disabled.%2$s', 'edds' ),
						'<strong>',
						'</strong>'
					) 
					. '<br />' .
					sprintf(
						/* translators: %1$s Opening code tag, do not translate. %2$s Closing code tag, do not translate. */
					 	__( 'To continue accepting credit card payments with Stripe please update the Recurring Payments extension to version %1$s2.9%2$s.', 'edds' ),
						'<code>',
						'</code>'
					),
					array(
...
function edds_admin_notices_print() {
	...
	try {
		...
		// Recurring 2.9.0 requirement.
		if ( defined( 'EDD_RECURRING_VERSION' ) && ! version_compare( EDD_RECURRING_VERSION, '2.8.8', '>' ) ) {
