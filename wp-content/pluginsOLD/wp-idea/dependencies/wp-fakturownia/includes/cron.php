...
if ( get_transient( 'bpmj_wpfa_doing_cron' ) ) {
	return;
}

set_transient( 'bpmj_wpfa_doing_cron', true, 55 ); // 55 sekund

$args = array(
	'posts_per_page' => 1,
	'post_type'      => 'bpmj_wp_fakturownia',
	'meta_key'       => 'fakturownia_status',
	'meta_value'     => 'pending',
	'orderby'        => 'ID',
	'order'          => 'ASC'
);
...
if ( ! empty( $invoices ) && is_array( $invoices ) ) {
	...
	foreach ( $invoices as $invoice ) {
		...
		// Próba wysyłki danych do Fakturownia.pl
		$invoice_object->send_invoice();
	}
}

delete_transient( 'bpmj_wpfa_doing_cron' );
