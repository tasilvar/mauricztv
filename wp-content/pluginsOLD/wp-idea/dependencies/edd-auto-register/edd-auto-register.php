...
if ( ! class_exists( 'EDD_Auto_Register' ) ) {
	...
	final class EDD_Auto_Register {
		...
		public function email_notifications( $user_id = 0, $user_data = array() ) {
            ...
            $message_sent_after_purchase = edd_get_option('bpmj_edd_arc_content', '' );
			...
			if ( ! $admin_email_disabled ) {
				...
				@wp_mail( $to_email, sprintf( __( '[%s] New User Registration', 'edd-auto-register' ), $blogname ), $message );
			}
			...
			// Email the user
			if ( ! $user_email_disabled &&  !empty($message_sent_after_purchase)) {
		...
		public function maybe_insert_user( $payment_id, $payment_data ) {
			...
			if ( ! is_user_logged_in() ) {
				...
				if( function_exists( 'did_action' ) && ! did_action( 'edd_create_payment' ) && ! did_action('edd_update_payment_details') ) {
		...
		public function create_user( $payment_data = array(), $payment_id = 0 ) {
			...
			if ( isset( $payment_data['user_info']['id'] ) && $payment_data['user_info']['id'] > 0 ) {
				return false;
			}
			...
			if ( ! is_wp_error( $user_id ) ) {
				...
				if ( true === $maybe_login_user ) {
                    $user = get_userdata( $user_id );
					edd_log_user_in( $user_id, $user->user_login, $user_args['user_pass'] );
